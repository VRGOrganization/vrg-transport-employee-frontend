...
export const enrollmentPeriodService = {
  ...
  list:      ()                      => http.get<EnrollmentPeriod[]>("/enrollment-period"),
  create:    (data: { startDate: string; endDate: string; totalSlots: number; licenseValidityMonths?: number }) =>
               http.post<EnrollmentPeriod>("/enrollment-period", data),
  update:    (id: string, data: Partial<{ startDate: string; endDate: string; totalSlots: number; licenseValidityMonths?: number }>) =>
