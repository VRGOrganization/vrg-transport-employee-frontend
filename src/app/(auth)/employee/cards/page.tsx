        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-headline text-2xl font-bold text-on-surface">Criação de Carteirinhas</h1>
                <p className="text-sm text-on-surface-variant">Visualize documentos, revise informações e aprove emissões.</p>
              </div>
            </div>
            ...
            <section className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-4 md:p-5">
              <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex rounded-xl border border-outline-variant bg-surface-container-low p-1 text-sm">
                  ...
                  <FilterButton active={filter === "with-card"} onClick={() => setFilter("with-card")}>Com carteirinha</FilterButton>
              ...
              <div className="mb-4 flex items-center justify-between gap-2 rounded-xl border border-outline-variant bg-surface p-2">
                <p className="text-xs text-on-surface-variant">
                  Selecionadas para lote: <strong className="text-on-surface">{selectedForBatch.length}</strong>
                </p>
                <Button
                  variant="outline"
                  size="sm"
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          ...
                          >
                            ...
                            <p className="truncate text-xs text-on-surface-variant">{student.institution ?? "Instituição não informada"}</p>
                          </button>
            ...
            <section className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-4 md:p-5">
              {!selected && (
                <div className="flex h-full min-h-96 flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-outline-variant bg-surface text-center text-on-surface-variant">
                  ...
                  <p className="max-w-xs text-xs">Você verá documentos, informações acadêmicas e poderá aprovar a criação da carteirinha.</p>
                </div>
              )}
                ...
                <div className="space-y-4">
                  <div className="rounded-xl border border-outline-variant bg-surface p-4">
                    ...
                    <div className="grid grid-cols-1 gap-2 text-xs text-on-surface-variant md:grid-cols-2">
                      ...
                      <p className="md:col-span-2"><strong className="text-on-surface">Instituição:</strong> {selected.institution ?? "—"}</p>
                    </div>
                  </div>
                    ...
                    {selected.schedule && selected.schedule.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {selected.schedule.map((item, index) => (
                          <span key={`${item.day}-${item.period}-${index}`} className="rounded-full bg-primary/10 px-2 py-1 text-xs text-primary">
                  ...
                  <div className="flex justify-end">
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        size="md"
                        icon={<Printer className="h-4 w-4" />}
                        disabled={!selected || !selectedLicensePreview || isPdfDataUrl(selectedLicensePreview) || printingSingle}
                      ...
                      <Button
                        ...
                        loading={approving}
                        disabled={!!currentLicense || !selected.institution || !selectedBus.trim()}
  ...
  return (
    <button
      ...
      className={`rounded-lg px-3 py-1.5 font-medium transition ${
        active ? "bg-surface-container-lowest text-on-surface shadow" : "text-on-surface-variant"
      }`}
    >
      {children}
    </button>
