...
export function DeactivateUniversityModal({
  ...
  const acronym = university.acronym;
  const canConfirm = inputValue === acronym && !loading;
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-on-surface-variant">
                Confirme a sigla da faculdade <span className="text-error">*</span>
              </label>
              <p className="text-[11px] text-on-surface-variant mb-1">
                Digite exatamente: <span className="font-bold text-on-surface select-all">{acronym}</span>
              </p>
              <input
                type="text"
                value={inputValue}
          ...
          <div className="px-8 py-4 bg-surface-container-low border-t border-outline-variant/20 flex justify-center gap-3 shrink-0">
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="px-6 py-2.5 rounded-full border-2 border-outline-variant text-on-surface-variant font-bold text-sm whitespace-nowrap hover:bg-surface-container-high transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-outline-variant/50"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={!canConfirm}
              className="flex items-center justify-center gap-2 whitespace-nowrap px-6 py-2.5 text-sm font-bold rounded-full bg-error text-white hover:bg-error/90 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer transition-colors focus:outline-none focus:ring-2 focus:ring-error/40"
            >
              {loading
